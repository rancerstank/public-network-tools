...
- **Structured JSON trace export & Chronological Multi-Host Combined outputs.** Checkboxes in the Debug Flow Options provide a clean 2x2 layout for Per-Host and Combined Text & JSON. When running across multiple firewalls simultaneously, the runner can produce both individual per-host files and consolidated multi-host files:
  - **Combined Chronological Text (`combined_ssh_debug_flow_<label>_<timestamp>.txt`)**: Interleaves raw trace lines across all targeted firewalls in strict ascending chronological order with `[hostname]` provenance tagging. Lines lacking timestamps (continuation lines, NAT details, routing decisions) attach directly after their preceding firewall line via a monotonic `(last_ts, host, line_idx)` sort key, preserving indentation.
  - **Combined Chronological JSON (`combined_ssh_debug_flow_<label>_<timestamp>.json`)**: Complete trace lifecycles are sorted chronologically across all firewalls by their initial trace start timestamp epoch. In addition, an "Export JSON" button in the GUI allows exporting the live log or trace history (including any active boolean/regex filters) at any time.
- **Symmetrical 2x2 Output Options Layout.** Replaced redundant master JSON toggle with clean independent options (Row 1: *Save Text Output*, *Save JSON*; Row 2: *Save Combined Text*, *Combined JSON*).
- **Fully encrypted session profiles (AES-256-GCM).** All session settings (target hostnames, IP addresses, subnets, ports, filter rules, usernames, passwords, private key passphrases, and output format preferences including `save_combined_text`) can be saved to a 100% encrypted profile file protected by a master passphrase using authenticated AES-256-GCM encryption with PBKDF2-HMAC-SHA256 key derivation. Zero plaintext network topology or credentials ever exist in saved profile files on disk.
...
- **Strict Running Chronological Ordering for Combined Text and JSON**: Multi-device sniffer aggregation parses packet timestamps across all FortiOS formats (ISO `YYYY-MM-DD`, slashes `YYYY/MM/DD` or `MM/DD/YYYY`, time-only `HH:MM:SS`, and relative offsets anchored to session start times, with ANSI escape sequence filtering). Combined text interleaves packet blocks in strict running chronological order across devices with `[hostname]` tagging, and Combined JSON strictly sorts all packet records by running epoch timestamp, re-indexing `packet_number` sequentially (1..N) across all targets.
