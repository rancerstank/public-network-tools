"""
diag_fgt_sniffer_pcap_v3.py

Standalone FortiGate Packet Sniffer & PCAP Tool over direct SSH using Paramiko.
Supersedes diag_fgt_sniffer_ssh_standalone_v2.py (archived) and v1.py (archived).

Key Capabilities & Enhancements in v3:
- Chronological Combined Text Capture:
  * Consolidates live sniffer packet streams from all targeted firewalls into a single combined
    text file (combined_ssh_sniffer_<label>_<timestamp>.txt).
  * Accurately parses timestamps across packet formats and executes a stable chronological sort
    across all devices with [hostname] tags.
- Combined Multi-Device PCAP Generation:
  * Automatically converts the chronological combined multi-device text stream into a single
    Wireshark-compatible PCAP file (combined_ssh_sniffer_<label>_<timestamp>.pcap).
  * Perfect for end-to-end packet flow analysis across transit firewalls in Wireshark.
- Symmetrical Output Format Options (3x3 Layout):
...
- Strict Reactive Checkbox Interlocking:
  * Disabling 'Save Text Output' automatically locks out 'Create PCAP'.
  * Disabling 'Save Combined Text' automatically locks out 'Create Combined PCAP'.
  * Per-Host and Combined JSON operate as independent toggles.
- High-Volume Live Stream Optimization:
  * Decoupled text rendering with batch-capped queue drainage (200 lines/cycle) and deferred
    see("end") scrolling, ensuring responsive GUI interactions even under heavy packet bursts.
- Wireshark / text2pcap Integration:
  * Auto-detection across Windows, macOS, and Linux; custom browse directory; real-time badge.
  * Standalone "Convert Log to PCAP" button to convert any past text capture to .pcap at any time.
  * Standardized strptime format ("%Y-%m-%d %H:%M:%S.") eliminating timestamp parsing errors.
- Dark Console Live Terminal:
  * High-contrast dark console (Consolas font on black background).
  * Whole-line per-host coloring using a 10-color terminal palette.
  * Real-time Boolean & Regex filtering (Show Matching, Hide Matching, Clear Filter).
- Structured JSON Packet Export:
  * Pure parser extracting 5-tuples, interface direction, protocol, and hex payload data.
  * Per-Host JSON and Combined Multi-Host JSON exports.
- Full-File Authenticated Profile Encryption (AES-256-GCM):
  * 100% encrypted session profiles on disk with PBKDF2-HMAC-SHA256 (supports v2 and v3 profiles).
- SSH Security & Verification:
  * Interactive Host Key Verification Wizard & Known Hosts Manager.
...
PROFILE_AAD = b"diag_fgt_sniffer_profile_v3"
PROFILE_AAD_V2 = b"diag_fgt_sniffer_profile_v2"
...
def parse_packet_blocks(raw_text: str, host: str = "", base_epoch: float = 0.0) -> list[dict[str, Any]]:
    ...
    for raw_line in raw_text.splitlines():
        ...
        if not stripped:
            continue
        m = TIMESTAMP_HEADER_RE.match(stripped)
        if m:
            if current_pkt:
            ...
            current_pkt["lines"].append(stripped)
    if current_pkt:
        packets.append(current_pkt)
    return packets
    ...
    for pkt in all_packets:
        ...
        for line in pkt["lines"]:
            if line.startswith("[") and "]" in line:
                ...
                formatted_lines.append(f"[{h}] {line}")
        formatted_lines.append("")
    
    return "\n".join(formatted_lines).strip(), len(all_packets)
...
class SnifferSshArgs:
    ...
    create_combined_pcap: bool = True
    save_json_separate: bool = True
    save_json_combined: bool = True
    timeout: int = DEFAULT_TIMEOUT_SECONDS

    @property
    def save_json_output(self) -> bool:
        return self.save_json_separate or self.save_json_combined
...
def save_session_profile(args: SnifferSshArgs, filepath: str, passphrase: str) -> None:
    ...
    raw_profile_data = {
        ...
        "create_combined_pcap": args.create_combined_pcap,
...
FIELD_HELP = {
    ...
    "save_text_output": "Save separate individual sniffer text output files (.txt) for each targeted firewall.",
    "create_pcap": "Automatically convert individual sniffer text outputs into Wireshark .pcap files (requires Save Text Output).",
    "save_combined_text": "Save a consolidated chronological text capture (.txt) combining all sniffer packets across all firewalls.",
    "create_combined_pcap": "Generate a single combined Wireshark .pcap file from the chronological combined text stream across all firewalls.",
    "save_json_output": "Export parsed packet data (5-tuples, interface, direction, protocols, hex payloads) to JSON.",
    "save_json_separate": "Export per-host parsed packet data (5-tuples, interface, direction, protocols, hex payloads) to JSON (.json).",
    "save_json_combined": "Export aggregated multi-host combined packet data to JSON (.json) when running across multiple target firewalls.",
...
class SnifferSshGui:
    ...
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("FortiGate Packet Sniffer & PCAP Tool (v3)")
    ...
    def build_ui(self) -> None:
        ...
        dur_lbl = ttk.Label(r2, text="Seconds", width=12, anchor="w")
    ...
    def update_output_states(self) -> None:
        """Enforce reactive interlocking across output format options.
        
        - If Save Text Output is unchecked -> lockout Create PCAP
        - If Save Combined Text is unchecked -> lockout Create Combined PCAP
        - Per-Host JSON and Combined JSON operate as independent toggles.
        """
        # 1. Per-Host Text & PCAP
        txt_on = bool(self.vars.get("save_text_output", tk.BooleanVar(value=True)).get())
        if hasattr(self, "pcap_cb") and self.pcap_cb:
            self.pcap_cb.configure(state="normal" if txt_on else "disabled")
        
        # 2. Combined Text & PCAP
        comb_txt_on = bool(self.vars.get("save_combined_text", tk.BooleanVar(value=True)).get())
        if hasattr(self, "comb_pcap_cb") and self.comb_pcap_cb:
            self.comb_pcap_cb.configure(state="normal" if comb_txt_on else "disabled")
    ...
    def _apply_profile(self, profile_data: dict) -> None:
        ...
        self.vars["create_combined_pcap"].set(profile_data.get("create_combined_pcap", True))
    ...
    def collect_args(self) -> SnifferSshArgs:
        ...
        args = SnifferSshArgs(
            ...
            create_pcap=bool(self.vars.get("create_pcap", tk.BooleanVar(value=True)).get()) if bool(self.vars.get("save_text_output", tk.BooleanVar(value=True)).get()) else False,
            save_combined_text=bool(self.vars.get("save_combined_text", tk.BooleanVar(value=True)).get()),
            create_combined_pcap=bool(self.vars.get("create_combined_pcap", tk.BooleanVar(value=True)).get()) if bool(self.vars.get("save_combined_text", tk.BooleanVar(value=True)).get()) else False,
    ...
    def start(self) -> None:
        ...
        if (args.create_pcap or args.create_combined_pcap) and not find_text2pcap(args.wireshark_path):
        ...
        def manager():
            try:
                ...
                if args.save_combined_text:
                    try:
                        combined_text_path, total_pkts = self.write_combined_text(args)
                        self.logger(LOG_INFO, f"Combined chronological text capture written ({total_pkts} pkts): {combined_text_path}")
                    except Exception as exc:
                        self.logger(LOG_ERROR, f"Failed to write combined text capture: {exc}")

                if args.save_combined_text and args.create_combined_pcap and combined_text_path:
                    try:
                        comb_pcap = self.write_combined_pcap(combined_text_path, args)
                        if comb_pcap:
                            self.logger(LOG_INFO, f"Combined multi-device Wireshark PCAP generated: {comb_pcap}")
                    except Exception as exc:
                        self.logger(LOG_ERROR, f"Failed to generate combined PCAP: {exc}")
    ...
    def write_combined_text(self, args: SnifferSshArgs) -> tuple[str | None, int]:
        """Write consolidated chronological sniffer packet capture across all targeted firewalls."""
        ...
        banner = [
            "=== FortiGate SSH Packet Sniffer Combined Capture (Chronological) ===",
            f"Hosts: {', '.join(args.hosts)}",
            f"SSH Port: {args.ssh_port}",
            f"Interface: {args.interface}",
            f"Verbose Level: {args.verbose}",
            f"Filter: {build_sniffer_filter(args) or 'none'}",
            f"Elapsed Seconds: {elapsed:.2f}",
            f"Total Combined Packets: {total_pkts}",
            f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            "",
            "=== Chronological Sniffer Output ===",
            merged_content,
        ]
        _write_text(path, "\n".join(banner))
        return path, total_pkts

    def write_combined_pcap(self, combined_text_path: str, args: SnifferSshArgs) -> str | None:
        """Create a single combined Wireshark .pcap file from the chronological combined text output."""
        pcap_path, pcap_err = create_pcap_file(combined_text_path, args.wireshark_path)
        if pcap_path:
            return pcap_path
        else:
            self.logger(LOG_WARN, f"Combined PCAP conversion skipped/failed: {pcap_err}")
            return None
